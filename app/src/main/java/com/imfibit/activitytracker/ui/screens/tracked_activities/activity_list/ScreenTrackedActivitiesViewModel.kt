...
@HiltViewModel
class ScreenTrackedActivitiesViewModel @Inject constructor(
    ...
    val data = invalidationStateFlow<Data?>(db, null, *activityTables){
    ...
    suspend fun createNewActivity(name: String, type: TrackedActivity.Type): Long {
        ...
        return rep.activityDAO.insert(activity)
    }

    fun onMoveActivity(from: Int, to: Int){
        val current = data.value ?: return
        data.value = current.copy(activities = current.activities.toMutableList().apply { swap(from, to) })
    ...
    fun onMoveGroup(from: Int, to: Int) {
        val current = data.value ?: return
        data.value = current.copy(groups = current.groups.toMutableList().apply { swap(from, to) })
    ...
    fun onDragEnd(){
        val current = data.value ?: return
        val activities = current.activities.mapIndexed{ index, item -> item.activity.copy(position = index) }

        val groups = current.groups.mapIndexed{ index, item -> item.copy(position = index) }
