...
@HiltViewModel(assistedFactory = ScreenTrackedActivityViewModel.Factory::class)
class ScreenTrackedActivityViewModel @AssistedInject constructor(
    ...
    @AssistedFactory
    interface Factory {
        fun create(activityId: Long): ScreenTrackedActivityViewModel
    }
    ...
    val data = invalidationStateFlow(db, null, *activityTables) {
        ...
        val months = rep.metricDAO.getMetricByMonth(
            ...
            val status = if (activity.goal.range == TimeRange.MONTHLY)
                if (activity.goal.value <= it.metric) MetricStatus.COMPLETED else MetricStatus.NOT_COMPLETED
            else
                MetricStatus.ACCENT
            ...
            MetricWidgetData(
                label = { it.from.month.name.take(3) },
                value = metric,
                status = status
            )
        }.reversed()
